...
// Sumbangan wang untuk Tabung Ihsan (Bantuan Kecemasan) — bayar terus via CHIP.
// Apabila berjaya: direkod dalam Kewangan ("Dana Tabung Ihsan") DAN dikreditkan
// ke baki Tabung Ihsan (bantuan_tabung) supaya paparan awam sentiasa tepat.
export async function mulaDermaTabungIhsan(input: {
  ...
  const site = siteUrl();
  const ref = `TABUNGIHSAN-${Date.now()}`;
  ...
  try {
    purchase = await ciptaPurchase({
      ...
      productName: "Sumbangan Tabung Ihsan — Surau Ar-Raudhah",
      reference: ref,
      success_redirect: `${site}/bantuan/selesai?ref=${encodeURIComponent(ref)}`,
      failure_redirect: `${site}/bantuan/selesai?ref=${encodeURIComponent(ref)}&gagal=1`,
