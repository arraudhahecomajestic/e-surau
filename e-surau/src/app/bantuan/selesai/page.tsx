...
export default function TabungIhsanSelesai({ searchParams }: { searchParams: { ref?: string; gagal?: string } }) {
    ...
    <div className="mx-auto max-w-lg rounded-xl bg-white p-8 text-center shadow-sm">
      ...
      <p className="mt-2 text-slate-600">
        {gagal
          ...
          : "Sumbangan anda ke Tabung Ihsan telah diterima. Ia akan membantu jiran kariah yang memerlukan. Semoga Allah membalas dengan kebaikan berlipat ganda."}
        {searchParams.ref ? <><br /><span className="text-xs text-slate-400">Rujukan: {searchParams.ref}</span></> : null}
      </p>
      <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
        <Link href="/bantuan" className="rounded-lg bg-surau px-5 py-2.5 font-semibold text-white hover:bg-surau-dark">{gagal ? "Cuba Semula" : "Kembali ke Tabung Ihsan"}</Link>
