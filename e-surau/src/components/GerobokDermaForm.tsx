...
import { mulaDermaGerobok } from "@/app/gerobok-prihatin/actions";

const PRESET = [10, 20, 30, 50, 100];

export default function GerobokDermaForm() {
  const [amt, setAmt] = useState<number>(30);
  ...
  async function bayar() {
    ...
    setSedang(true);
    const res = await mulaDermaGerobok({ amount: nilai, nama, emel });
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {sedang ? "Menyambung ke pembayaran…" : `Sumbang RM${nilai || 0} Sekarang`}
      </button>
      <p className="text-center text-xs text-slate-400">Bayaran diproses selamat melalui CHIP (FPX / kad / e-wallet). Resit dihantar ke e-mel anda.</p>
    </div>
  );
}
