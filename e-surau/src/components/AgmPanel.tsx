...
import { simpanAgm, tandaHadir, padamHadir, tambahUsul, kemasUndi, padamUsul } from "@/app/admin/agm/actions";

type Agm = { id: string; tajuk: string; tahun: number; tarikh: string | null; masa: string | null; tempat: string | null; kuorum: number; atur_cara: string | null; status: string };
type Hadir = { id: string; ahli_id: string | null; nama: string; no_ahli: string | null; masa_daftar: string };
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      {agm && !edit ? (
        <div className="space-y-1 text-sm text-slate-700">
          ...
          <div>📍 {agm.tempat ?? "—"}</div>
          <div>👥 Kuorum diperlukan: <b>{agm.kuorum || "—"}</b></div>
        ...
        <form action={simpanAgm} className="space-y-3">
          ...
          <div className="flex gap-2">
            <button type="submit" className="rounded-lg bg-surau px-5 py-2 text-sm font-bold text-white hover:bg-surau-dark">Simpan</button>
            {agm && <button type="button" onClick={() => setEdit(false)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm text-slate-600">Batal</button>}
          </div>
        </form>
      )}
    </section>
...
function DaftarHadir({ agm, hadir, ahli }: { agm: Agm; hadir: Hadir[]; ahli: Ahli[] }) {
  ...
  async function daftar(a: Ahli) {
    setBusy(true); await tandaHadir(agm.id, a.id, a.nama, a.no_ahli); setBusy(false); setCari(""); router.refresh();
  }
  async function daftarWalkIn() {
    if (!nama.trim()) return;
    setBusy(true); await tandaHadir(agm.id, null, nama, null); setBusy(false); setNama(""); router.refresh();
  }
          ...
          <ol className="divide-y divide-slate-100 rounded-lg border border-slate-100 text-sm">
            {hadir.map((h, i) => (
              <li key={h.id} className="flex items-center justify-between gap-3 px-3 py-1.5">
                <span className="min-w-0 truncate"><span className="mr-2 text-slate-400">{i + 1}.</span>{h.nama}{h.no_ahli ? <span className="ml-2 text-xs text-slate-400">{h.no_ahli}</span> : ""}{!h.ahli_id && <span className="ml-2 rounded bg-slate-100 px-1.5 text-[10px] text-slate-500">luar</span>}</span>
...
function UsulUndian({ agm, usul }: { agm: Agm; usul: Usul[] }) {
  ...
  async function tambah() {
    ...
    setBusy(true); await tambahUsul(agm.id, tajuk, ket); setBusy(false); setTajuk(""); setKet(""); router.refresh();
  }
  ...
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      ...
      <div className="mb-4 space-y-2 rounded-lg bg-slate-50 p-3">
        ...
        <button disabled={busy} onClick={tambah} className="rounded-lg bg-surau px-4 py-2 text-sm font-semibold text-white hover:bg-surau-dark disabled:opacity-50">+ Tambah Usul</button>
      </div>
...
function UsulRow({ u, onDone }: { u: Usul; onDone: () => void }) {
  ...
  async function simpan() { setBusy(true); await kemasUndi(u.id, s, t, b, kep, cat); setBusy(false); setBuka(false); onDone(); }
    ...
    <div className="rounded-lg border border-slate-200 p-3">
      ...
      {buka && (
        <div className="mt-3 border-t border-slate-100 pt-3">
          ...
          <div className="mt-3 flex gap-2">
            <button disabled={busy} onClick={simpan} className="rounded-lg bg-surau px-4 py-1.5 text-xs font-bold text-white hover:bg-surau-dark disabled:opacity-50">Simpan Undi</button>
